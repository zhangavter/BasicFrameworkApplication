...
import zhang.acfun.com.basicframeworklib.network.HttpUtil
...
object SecurityUtil {
    ...
    //能够抓包，默认不行
    @JvmStatic
    var needCapturePacket = Constants.EnDebug
    ...
    private fun openVPNSettings() {
        try {
            Constants.getContext().startActivity(
    ...
    private fun showVpnCloseDialog() {
        if (!haveShowVpnDialog) {
            try {
                val stackRecord = AppManager.getInstance().topActivity ?: return
